...
class Bluetoothctl:
    ...
    def is_json(self, myjson):
        try:
            ...
            array_object = np.array(json_object)
            b_data = np.choose([1, 2, 3, 4, 5, 6], array_object)
            mean_data = np.array([0, 0, 0, 0, 0, 0])
            choosen_data = b_data == 0
            actual_data = mean_data == 0
            if (choosen_data == actual_data).all():
                return None
            else:
                return json_object
        except ValueError:
...
def insertLOG(database, target_address, data):
    def check_data(datas):
        try:
            dt_obj = datetime(datas[6], datas[7], datas[8], datas[9], datas[10], datas[11])
    ...
    try:
        ...
        list = {
            "uuid": target_address,
            "sensor": [data[1], data[2], data[3], data[4], data[5], data[6]],
            "activity": "stay",
